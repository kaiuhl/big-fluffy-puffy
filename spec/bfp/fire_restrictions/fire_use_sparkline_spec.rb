...
RSpec.describe BFP::FireRestrictions::FireUseSparkline do
  ...
  it "renders a compact one-row fire-use SVG with accessible policy detail" do
    ...
    expect(html).to include("--fire-use-width: 230px")
    ...
    expect(html).to include(">Campfire</span>")
    expect(html).to include(">Gas</span>")
    expect(html).to include(">Solid fuel</span>")
    expect(html).not_to include(">Alcohol</span>")
