...
RSpec.describe RodaApp do
  ...
  it "serves the fire restrictions page shell" do
    ...
    expect(last_response.body).to include('href="/styles/site.css?v=20260516-icons-only"')
    expect(last_response.body).to include('src="/vendor/leaflet/leaflet.js"')
    expect(last_response.body).to include('src="/scripts/fire-restrictions.js?v=20260516-map-zoom-in"')
  ...
  it "serves the fire restrictions search script" do
    ...
    expect(last_response.body).to include("DEFAULT_FIT_ZOOM_OFFSET = 1")
    expect(last_response.body).to include("zoomInAfterFit(map)")
    expect(last_response.body).to include('typeof mapMaxZoom !== "number"')
